...
int s21_add(s21_big_decimal value_1, s21_big_decimal value_2, s21_big_decimal *result) {
  ...
  if (sign_1 == sign_2) {
    int add = 0;
    for(int i = 0; i < 171; i++) {
      int val1 = s21_big_decimal_check_bit(value_1, i);
      int val2 = s21_big_decimal_check_bit(value_2, i);
      if ((val1 + val2 + add) >= 2) {
        if(add)
          *result = s21_big_decimal_set_bit(*result, i);
        else
          *result = s21_big_decimal_unset_bit(*result, i);
        add = 1;
      } else {
        add = 0;
        *result = s21_big_decimal_set_bit(*result, val1 + val2);
      }
    }
  } else {
...
// int s21_sub(s21_decimal value_1, s21_decimal value_2, s21_decimal *result) { return 0;}
